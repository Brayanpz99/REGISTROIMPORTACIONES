...
import { InsurancePolicyBadge } from '@/components/importaciones/insurance-policy-badge'
...
export default async function ImportacionDetallePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const policyState = getInsurancePolicyState(data.insuranceExpiresAt)
  const policyAlertText = policyState.status === 'warning' && policyState.daysRemaining !== null
    ? `Vence en ${policyState.daysRemaining} día(s)`
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-soft">
        <div className="mb-4 flex items-center justify-between gap-3">
          <h2 className="text-xl font-semibold text-slate-950">Documentación clave</h2>
          <InsurancePolicyBadge status={policyState.status} />
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <InfoRow label="DAI" value={data.daiNumber ?? 'No registrado'} />
          <InfoRow label="Fecha DAI" value={data.daiDate ? formatDate(data.daiDate) : 'No registrada'} />
          <InfoRow label="Póliza" value={data.insurancePolicyNumber ?? 'No registrada'} />
          <InfoRow label="Aseguradora" value={data.insuranceCompany ?? 'No registrada'} />
          <InfoRow label="Fecha emisión" value={data.insuranceIssuedAt ? formatDate(data.insuranceIssuedAt) : 'No registrada'} />
          <InfoRow label="Fecha vencimiento" value={data.insuranceExpiresAt ? formatDate(data.insuranceExpiresAt) : 'No registrada'} />
        </div>
        {policyAlertText ? <p className="mt-4 text-sm font-semibold text-amber-700">{policyAlertText}</p> : null}
      </section>
